...
use rust_bert_score::{cli::TokenizerSpec, core::{
    choose_hf_or_pathed_tokenizer, BERTScorerBuilder, BERTScorerConfig
}, BERTScorer};

// use rust_bert::pipelines::common::ModelType;
// use rust_bert_score::{baseline::BaselineScores, BERTScorerBuilder, BERTScorerConfig};
use tch::Device;
...
fn test_basic_scoring() {
    ...
    let spec : TokenizerSpec = TokenizerSpec {
        pretrained: Some("roberta-large".to_string()),
        vocab: None,
        merges: None,
    };

    let Ok((path, merges)) = choose_hf_or_pathed_tokenizer(&spec) else { panic!("Failed to choose tokenizer paths") };
    ...
    // Test data
    let candidates = vec!["The cat sat on the mat.", "A dog ran in the park."];
...
fn test_idf_weighting() {
    use rust_bert_score::core::IdfDict;
...
fn test_multi_reference_scoring() {
    ...
    use rust_bert_score::core::BERTScoreResult;
