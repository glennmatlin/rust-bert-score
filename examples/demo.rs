...
use rust_bert_score::{
    BERTScorer, BERTScorerBuilder, BERTScorerConfig, BERTScoreResult,
...
fn demo_similarity_computation() -> anyhow::Result<()> {
    println!("1. Basic Similarity Computation Demo");
    println!("------------------------------------");
...
fn demo_baseline_rescaling() -> anyhow::Result<()> {
    ...
    for (i, &(p, r, f1)) in raw_scores.iter().enumerate() {
        if let Some((p_scaled, r_scaled, f1_scaled)) = 
            ...
            println!("    Pair {}: ({:.3}, {:.3}, {:.3}) → ({:.3}, {:.3}, {:.3})",
                     i + 1, p, r, f1, p_scaled, r_scaled, f1_scaled);
        }
    }
    println!();
...
fn example_with_special_tokens() -> anyhow::Result<()> {
    ...
    let mask = create_scoring_mask(&token_ids, &special_token_ids, length);
...
fn example_multi_reference() -> anyhow::Result<()> {
    ...
    let candidate_results = vec![
        BERTScoreResult { precision: 0.85, recall: 0.82, f1: 0.834 },
        BERTScoreResult { precision: 0.90, recall: 0.88, f1: 0.889 }, // Best
        BERTScoreResult { precision: 0.87, recall: 0.84, f1: 0.854 },
    ];
