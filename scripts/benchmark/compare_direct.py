...
# Allow specifying which files to compare via command line
import sys
if len(sys.argv) > 2:
    py_file = sys.argv[1]
    ...
    # Default files
    rust_file = "reports/direct_scores_rust_real.csv"  # Use real Rust output if available
    if not os.path.exists(rust_file):
        rust_file = "reports/direct_scores_rust.csv"  # Fall back to mock if needed
