...
pub fn compute_bertscore(
    ...
    let (precision, recall) = if let Some((cand_idf, ref_idf)) = idf_weights {
        compute_weighted_scores(&masked_similarity, candidate_mask, reference_mask, cand_idf, ref_idf)?
    } else {
        compute_unweighted_scores(&masked_similarity, candidate_mask, reference_mask)?
    };
    ...
    Ok(BERTScoreResult { precision, recall, f1 })
}

/// Normalizes embeddings to unit length (L2 normalization).
fn normalize_embeddings(embeddings: &Tensor) -> Result<Tensor> {
...
fn compute_similarity_matrix(candidate_norm: &Tensor, reference_norm: &Tensor) -> Result<Tensor> {
    // Matrix multiplication: (seq_len_c × hidden) @ (hidden × seq_len_r) = (seq_len_c × seq_len_r)
    Ok(candidate_norm.matmul(&reference_norm.transpose(0, 1)))
}
...
fn apply_masks(
    ...
    let masked = similarity_matrix.where_self(
        &combined_mask,
        &Tensor::full_like(similarity_matrix, f64::NEG_INFINITY),
    );
...
fn compute_unweighted_scores(
    ...
    let valid_cand_count = candidate_mask.sum(tch::Kind::Float);
    let precision = if valid_cand_count.double_value(&[]) > 0.0 {
        ((&max_cand_sims * candidate_mask).sum(tch::Kind::Float) / &valid_cand_count)
            .double_value(&[]) as f32
    } else {
        0.0
    };
    ...
    let valid_ref_count = reference_mask.sum(tch::Kind::Float);
    let recall = if valid_ref_count.double_value(&[]) > 0.0 {
        ((&max_ref_sims * reference_mask).sum(tch::Kind::Float) / &valid_ref_count)
            .double_value(&[]) as f32
    } else {
        0.0
    };
...
/// Computes IDF-weighted precision and recall scores.
fn compute_weighted_scores(
    masked_similarity: &Tensor,
    candidate_mask: &Tensor,
    reference_mask: &Tensor,
    candidate_idf: &Tensor,
    reference_idf: &Tensor,
) -> Result<(f32, f32)> {
    // Precision: weighted average using candidate IDF weights
    let max_cand_sims = masked_similarity.max_dim(1, false).0;
    let weighted_cand_sims = &max_cand_sims * candidate_idf * candidate_mask;
    let cand_idf_sum = (candidate_idf * candidate_mask).sum(tch::Kind::Float);
    let precision = if cand_idf_sum.double_value(&[]) > 0.0 {
        (weighted_cand_sims.sum(tch::Kind::Float) / &cand_idf_sum).double_value(&[]) as f32
    } else {
        0.0
    };
    
    // Recall: weighted average using reference IDF weights
    let max_ref_sims = masked_similarity.max_dim(0, false).0;
    let weighted_ref_sims = &max_ref_sims * reference_idf * reference_mask;
    let ref_idf_sum = (reference_idf * reference_mask).sum(tch::Kind::Float);
    let recall = if ref_idf_sum.double_value(&[]) > 0.0 {
        (weighted_ref_sims.sum(tch::Kind::Float) / &ref_idf_sum).double_value(&[]) as f32
    } else {
        0.0
    };
...
pub fn create_scoring_mask(
    token_ids: &[i64],
    special_token_ids: &[i64],
    length: usize,
) -> Tensor {
...
mod tests {
    ...
    fn test_normalize_embeddings() {
        ...
        let expected = Tensor::from_slice2(&[
            &[0.6, 0.8],
            &[1.0, 0.0],
            &[0.0, 0.0],
        ]);
    ...
    fn test_compute_similarity_matrix() {
        let cand = Tensor::from_slice2(&[
            &[1.0, 0.0],
            &[0.0, 1.0],
        ]);
        let ref_ = Tensor::from_slice2(&[
            &[1.0, 0.0],
            &[0.707, 0.707],
            &[0.0, 1.0],
        ]);
        
        let sim_matrix = compute_similarity_matrix(&cand, &ref_).unwrap();
        let expected = Tensor::from_slice2(&[
            &[1.0, 0.707, 0.0],
            &[0.0, 0.707, 1.0],
        ]);
