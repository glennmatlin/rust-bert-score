...
impl BaselineManager {
    ...
    pub fn with_defaults() -> Self {
        ...
        // English baselines
        manager.add_baseline("bert-base-uncased", "en", BaselineScores::new(0.85, 0.85, 0.85));
        manager.add_baseline("roberta-large", "en", BaselineScores::new(0.87, 0.87, 0.87));
        manager.add_baseline("microsoft/deberta-xlarge-mnli", "en", BaselineScores::new(0.88, 0.88, 0.88));
        
        // Multilingual baselines
        manager.add_baseline("bert-base-multilingual-cased", "zh", BaselineScores::new(0.84, 0.84, 0.84));
        manager.add_baseline("xlm-roberta-large", "en", BaselineScores::new(0.86, 0.86, 0.86));
        manager.add_baseline("xlm-roberta-large", "zh", BaselineScores::new(0.85, 0.85, 0.85));
    ...
    pub fn add_baseline(&mut self, model: &str, language: &str, scores: BaselineScores) {
        self.baselines.insert((model.to_string(), language.to_string()), scores);
    }
    
    /// Gets baseline scores for a model and language.
    pub fn get_baseline(&self, model: &str, language: &str) -> Option<&BaselineScores> {
        self.baselines.get(&(model.to_string(), language.to_string()))
    }
